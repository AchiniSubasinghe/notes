...
Request eka yana widiha🡪 Domain-cloudfare-Ec2-Nginx

Apache tom cat run wena server eka :8080

Reverse proxy mechanism

Nginx ekata api damama api.samudi damama nginx eke run wenne local host 8080 kiyala

Mekata step tika,

Prompt= Create nginx config file for api.achini.me to reverse proxy localhost:8080
...
File eka open karanna oni nm, Sudo nano file name .
...
Ekata solution-body parser middleware ekt new version wlaata enw
...
Connection pool=connection gdk
...
Data yanna and ganna use krnn json object

Jwt =json token give after login –meh spring security module eke athule na ,express js eketh default ema ekk na

Node js wala =json web token is used
...
secret key(user and developer)

jsonwebtoken and bycrypt

token eka temparaly store krna widi, web nm local storage,session

mobile nm async

docker desktop , ubuntu , docker hub account
